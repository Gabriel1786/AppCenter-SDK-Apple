...
- (BOOL)loadLogsWithGroupId:(NSString *)groupId
  ...
  // Get target keys from batches.
  if (targetKeys.count > 0) {
    [condition appendFormat:@" AND ( \"%@\" IS NULL", kMSTargetKeyColumnName];
    for (NSUInteger i = 0; i < targetKeys.count; i++) {
      [condition appendFormat:@" OR \"%@\" = '%@'", kMSTargetKeyColumnName, targetKeys[i]];
    }
